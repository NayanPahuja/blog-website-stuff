...
export const isColumnSelected = (columnIndex: number) => (selection: Selection) => {
  if (isCellSelection(selection)) {
    const map = TableMap.get(selection.$anchorCell.node(-1))
    return isRectSelected({ left: columnIndex, right: columnIndex + 1, top: 0, bottom: map.height })(selection)
  }
  return false
}
...
export const isRowSelected = (rowIndex: number) => (selection: Selection) => {
  if (isCellSelection(selection)) {
    const map = TableMap.get(selection.$anchorCell.node(-1))
    return isRectSelected({ left: 0, right: map.width, top: rowIndex, bottom: rowIndex + 1 })(selection)
  }
  return false
}
...
export const isTableSelected = (selection: Selection) => {
  if (isCellSelection(selection)) {
    const map = TableMap.get(selection.$anchorCell.node(-1))
    return isRectSelected({ left: 0, right: map.width, top: 0, bottom: map.height })(selection)
  }
  return false
}
  ...
  if (table) {
    ...
    if (index >= 0 && index < (isRowSelection ? map.height : map.width)) {
      ...
      const cellsInFirstRow = map.cellsInRect({
        left, top, right: isRowSelection ? right : left + 1, bottom: isRowSelection ? top + 1 : bottom,
      })
      const cellsInLastRow =
        bottom - top === 1
          ? cellsInFirstRow
          : map.cellsInRect({
              left: isRowSelection ? left : right - 1, top: isRowSelection ? bottom - 1 : top, right, bottom,
            })
...
export const isColumnGripSelected = ({ editor, view, state, from }: {
  editor: Editor; view: EditorView; state: EditorState; from: number
}) => {
  const domAtPos = view.domAtPos(from).node as HTMLElement
  const nodeDOM = view.nodeDOM(from) as HTMLElement
  const node = nodeDOM || domAtPos
  if (!editor.isActive('table') || !node || isTableSelected(state.selection)) return false
  let container = node
  while (container && !['TD', 'TH'].includes(container.tagName)) container = container.parentElement!
...
export const isRowActiveFromSelection = ({ editor, view, state, from }: {
  editor: Editor; view: EditorView; state: EditorState; from: number
}) => {
  ...
  while (anchorCell && !['TD', 'TH'].includes(anchorCell.tagName)) anchorCell = anchorCell.parentElement!
  if (!anchorCell) return false
  const anchorRow = anchorCell.closest('tr')
  return !!anchorRow && anchorRow === rowEl
}

export const isColumnActiveFromSelection = ({ editor, view, state, from }: {
  editor: Editor; view: EditorView; state: EditorState; from: number
}) => {
  if (!editor.isActive('table')) return false
  const domAtPos = view.domAtPos(from).node as HTMLElement
